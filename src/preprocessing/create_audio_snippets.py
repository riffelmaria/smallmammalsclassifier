...
def parse_mm_ss(value: str) -> timedelta:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Time must be in the format mm:ss (e.g., 05:30).")
...
def write_audiosegments(audio_file, duration, new_filename) -> None:
    ...
    for start in range(0, audio_length, duration*sr):
        ...
        sf.write(new_filename.format(start=int(start_frame/sr), end=int(end_frame/sr)), segment, sr)
    print(f"Number of snippet files: {len(range(0, audio_length, duration*sr))}")


def process_folder(folder_path: str, folder_to_snippets: str, duration: timedelta) -> None:
    for audiofile in walk_wav(folder_path, ignore_path=folder_to_snippets):
        write_audiosegments(
            audio_file=audiofile, 
            duration=int(duration.total_seconds()), 
            new_filename=f"{folder_to_snippets}/{Path(audiofile).parent.stem}_{Path(audiofile).stem}" + "_{start}_{end}.WAV")
